	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if (view.isMultiDayEvent(event)) { // if the event appears to span more than one day...
			...
			if (seg.isStart || seg.isEnd) {
				...
				startTimeText = this.getEventTimeText(seg, null, this.view.options.showMinutes); // displayEnd=false
			}
		} else {
			// Display the normal time text for the *event's* times
			timeText = this.getEventTimeText(event);
			fullTimeText = this.getEventTimeText(event, 'LT');
			startTimeText = this.getEventTimeText(event, null, this.view.options.showMinutes); // displayEnd=false
		}
...
var AgendaView = FC.AgendaView = View.extend({
	...
	initialize: function() {
		...
		if (this.options.showAllDay) { // should we display the "all-day" area?
